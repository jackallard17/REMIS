...
TOGGLE(injectorMode, setInjectorMode, "Mode: ", doNothing, noEvent, noStyle //,doExit,enterEvent,noStyle
       ,
       VALUE("Dose", 0, doNothing, noEvent), VALUE("Cont.", 1, doNothing, noEvent), VALUE("Toggle.", 2, doNothing, noEvent));
...
MENU(subMenu, "", showEvent, anyEvent, noStyle, OP("Sub1", showEvent, anyEvent), OP("Sub2", showEvent, anyEvent), OP("Sub3", showEvent, anyEvent), altOP(altPrompt, "", showEvent, anyEvent), EXIT("<Back"));

char *constMEM hexDigit MEMMODE = "0123456789ABCDEF";
char *constMEM hexNr[] MEMMODE = {"0", "x", hexDigit, hexDigit};
char buf1[] = "0x11";

MENU(mainMenu, "Main menu", doNothing, noEvent, noStyle, FIELD(test, "Speed: ", "RPM", 0, 500, 10, 1, doNothing, noEvent, noStyle), OP("================", doNothing, enterEvent), SUBMENU(setInjectorMode), OP("Calibrate", doCalibration, enterEvent), EXIT("<Back"));
...
void loop()
{
  ...
  else if (digitalRead(TRIGGER) == HIGH && injectorMode == 0)
  {
    runPump();
  }
  else if (digitalRead(TRIGGER) == HIGH && injectorMode == 1)
  {
    int steps = 0;
